...
class SlickNotesStorage {
    ...
    func readNotes(withPredicate: NSPredicate?, sortDescriptors: [NSSortDescriptor]? = nil) -> [SlickNotes]?{
        if managedContextHasBeenSet {
            return SlickNotesCoreDataHelper.readNotesFromCoreData(fromManagedObjectContext: self.managedObjectContext, predicate: withPredicate, sortDescriptors: sortDescriptors)
        }
        return nil
    }
